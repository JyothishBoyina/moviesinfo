...
function renderGrid(selector, list, type = 'movie') {
  ...
  el.innerHTML = list.map(m => {
    ...
    const rating = (typeof m.vote_average === 'number' && m.vote_average !== null) ? m.vote_average.toFixed(1) : 'N/A';
...
async function renderSearch(params) {
  setLoading(true);
  try {
    if (!params.query) { window.location.hash = '#/'; return; }
...
searchInput.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    ...
    if (!q) window.location.hash = '#/'; else window.location.hash = `#/search?query=${encodeURIComponent(q)}&page=1`;
  }
});
